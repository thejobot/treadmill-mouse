...
typedef struct {
    ...
    uint16_t gyro_units_per_deg; // Raw gyro counts per degree/second. 0 = unstated,
                                // in which case consumers assume the Sony convention
                                // (1024). A Joy-Con is ~16, so anything that turns
                                // gyro into a real-world rate has to read this rather
                                // than assume.
...
static inline void init_input_event(input_event_t* event) {
    ...
    event->gyro_units_per_deg = 0;  // Unstated: consumers fall back to the Sony 1024
    for (int i = 0; i < 3; i++) {
        event->accel[i] = 0;
        event->gyro[i] = 0;
    }
