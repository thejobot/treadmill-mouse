...
// Max gap between clicks for a double or triple click.
//
// The BOOTSEL button cannot be polled freely — each read stops the flash and
// disables interrupts — so it is sampled on a timer rather than every loop.
// A 300 ms window is only a handful of samples wide, which makes a triple
// click on it a matter of luck: presses land between samples and are simply
// never seen. The window scales with the sample rate so the button is
// physically hittable instead.
#ifdef USE_BOOTSEL_BUTTON
#define BUTTON_DOUBLE_CLICK_MS  700
#else
