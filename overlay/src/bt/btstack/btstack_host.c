...
void btstack_host_start_timed_scan(uint32_t timeout_ms)
{
    ...
    // Start on LIAC. A timed scan is only ever asked for because somebody is
    // holding a sync button, and a pad in sync mode is limited discoverable —
    // Joy-Cons, Wiimotes, Wii U Pro. Opening on GIAC wastes the first 6.4s
    // round on exactly the devices that are not asking to be found.
    classic_state.use_liac = true;
...
#ifndef CONFIG_USB2BLE
            // Restart inquiry after it completes, toggling GIAC/LIAC so every
            // device type gets a look in.
            //
            // The state test alone was not enough. Connecting a controller
            // takes the host out of BLE_STATE_SCANNING, so once one pad was
            // paired this stopped firing and each button press bought exactly
            // one 6.4s round — always on whichever LAP was last left set. A
            // second pad held in sync mode is LIMITED discoverable, so if that
            // stuck round was GIAC it could never be found, no matter how many
            // times the button was pressed. That is why pairing a second
            // Joy-Con looked impossible while the first went on easily.
            //
            // An open timed-scan window now counts as well, so a deliberate
            // "look for another controller" press keeps alternating for its
            // full duration even with a pad already connected.
            bool window_open = (scan_timeout_end != 0 &&
                                (int32_t)(btstack_run_loop_get_time_ms() - scan_timeout_end) < 0);
            if (hid_state.state == BLE_STATE_SCANNING || window_open) {
        ...
        case HCI_EVENT_PACKET: {
            ...
            if (event_type == L2CAP_EVENT_CHANNEL_OPENED) {
                ...
                if (psm == PSM_HID_CONTROL && wiimote_conn.state == WIIMOTE_STATE_W4_CONTROL_CONNECTED) {
                    ...
                    wiimote_conn.state = WIIMOTE_STATE_CONNECTED;
                    classic_state.pending_hid_connect = false;
                    // Setup is complete, so nothing is pending. Only the
                    // hid_host path cleared pending_valid (in
                    // HID_SUBEVENT_CONNECTION_OPENED); this direct-L2CAP path
                    // never fires that event, so the flag stayed true for the
                    // life of the session and btstack_host_start_timed_scan()
                    // refused every button press with "Timed scan ignored" —
                    // which is why a second Joy-Con could never be paired
                    // while the first was connected.
                    classic_state.pending_valid = false;
                    classic_state.pending_outgoing = false;
