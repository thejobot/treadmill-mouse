...
// IMU
// ============================================================================
// The 0x30 report already carries motion once subcommand 0x40 has switched the
// IMU on: three 12-byte frames sampled 5 ms apart, newest first, each one six
// little-endian int16s in the order accel X/Y/Z then gyro X/Y/Z. Accel and gyro
// share the sensor's axes, which is what lets the gravity-based "any angle"
// pointer mode work — it needs gyro[i] and accel[i] to mean the same axis.
//
// Only the newest frame is used. The older two exist so a host polling slower
// than 5 ms can reconstruct the motion in between; a pointer that emits on
// every report has no use for history.
#define SWITCH_IMU_OFFSET               13
#define SWITCH_IMU_FRAME_LEN            12

// ICM-20600 at its Switch defaults: gyro ±2000 dps at 0.06103 dps per count,
// accel ±8 g at 0.000244 g per count. 1/0.06103 is 16.38 counts per degree/sec
// and this field is whole counts, so the pointer runs 2.4% slow — well under
// what the sensitivity setting is adjusted by feel anyway.
#define SWITCH_GYRO_UNITS_PER_DEG       16
#define SWITCH_GYRO_RANGE_DPS           2000
#define SWITCH_ACCEL_RANGE_MILLI_G      8000
...
typedef enum {
    ...
    SWITCH_STATE_ENABLE_IMU,        // Send enable IMU (0x40 → 0x01)
...
static void switch_process_report(bthid_device_t* device, const uint8_t* data, uint16_t len)
{
    ...
    if (report_id == SWITCH_REPORT_INPUT_STANDARD && len >= 13) {
        ...
        // Motion. Absent until subcommand 0x40 lands, and a report that is too
        // short to hold a frame leaves the previous sample alone rather than
        // publishing zeros as if the controller had gone still.
        if (len >= SWITCH_IMU_OFFSET + SWITCH_IMU_FRAME_LEN) {
            const uint8_t* imu = data + SWITCH_IMU_OFFSET;
            for (int axis = 0; axis < 3; axis++) {
                int o = axis * 2;
                sw->event.accel[axis] = (int16_t)(imu[o] | (imu[o + 1] << 8));
                sw->event.gyro[axis]  = (int16_t)(imu[o + 6] | (imu[o + 7] << 8));
            }
            sw->event.gyro_range = SWITCH_GYRO_RANGE_DPS;
            sw->event.accel_range = SWITCH_ACCEL_RANGE_MILLI_G;
            sw->event.gyro_units_per_deg = SWITCH_GYRO_UNITS_PER_DEG;
    ...
    switch (sw->init_state) {
        ...
        case SWITCH_STATE_SET_PLAYER_LED:
            if (now - sw->init_time >= SWITCH_INIT_DELAY_MS) {
                ...
                uint8_t enable = 0x01;
                switch_send_subcommand(device, SWITCH_SUBCMD_ENABLE_IMU, &enable, 1);
