...
static void on_button_event(button_event_t event)
{
    switch (event) {
        ...
        case BUTTON_EVENT_TRIPLE_CLICK:
            // Triple-click drops into the UF2 bootloader, so a new build can go
            // on without unplugging the dongle to hold the button down while it
            // powers up. Upstream used this for usbd_reset_to_hid(); the mode
            // it reset to is the only one this build ships, so nothing is lost.
            //
            // This never returns. The board reappears as a drive called RP2350
            // and stops being a mouse until a file lands on it.
            printf("[app:bt2usb] Triple-click - entering bootloader...\n");
