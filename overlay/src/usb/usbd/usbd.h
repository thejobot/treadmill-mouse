...
// True when KB/Mouse mode is enumerating a mouse-only identity: one HID
// interface, no keyboard, no gamepad. Fixed at boot from flash.
bool usbd_pure_mouse_active(void);

// HID instance index carrying the mouse, which differs between the composite
// and mouse-only descriptors. TinyUSB numbers instances by descriptor order,
// not by interface number, so this must be asked rather than assumed.
